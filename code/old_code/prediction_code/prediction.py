...
from skopt.space import Real, Integer, Uniform
...
def train_grid(model,X,y):
    
    rkf_inner = KFold(X,y,n_splits=INNER_FOLDS)
    param_space = {
        'eta':Real(0.01,1.0,prior='log-uniform'),
        'max_depth':Integer(1,50),
        'n_estimators':Integer(50,500),
        'subsample':Uniform(0.25,1)
    }
    gcv = BayesSearchCV(estimator=model,
...
# Main code
if len(sys.argv) != 4:
    print("Usage: python3 code.py split_vector.csv features.csv labels.csv")
    sys.exit(1)
